...
export default function ChatBox({ open, onClose, user }) {
  ...
  useEffect(() => { if (open) api('/cuoc-tro-chuyens', { headers: { Accept: 'application/json' } }).then(r => r.ok ? r.json() : null).then(body => { setConversations(body?.data || []); setActive(current => current || body?.data?.[0]?.id || null) }) }, [open])
